...
def _(get_viz_ds, mo):
    ...
    if _meta is None:
        ...
        viz_x        = mo.ui.range_slider(start=0, stop=1, value=[0, 1], label="X range", show_value=True)
        viz_y        = mo.ui.range_slider(start=0, stop=1, value=[0, 1], label="Y range", show_value=True)
    ...
    return viz_colormap, viz_depth, viz_field, viz_timestep, viz_x, viz_y
...
def _(
    ...
    viz_load_button, viz_timestep, viz_url, viz_x, viz_y,
):
    """Render a horizontal 2-D slice from the remote dataset — auto-updates on any control change."""
    _meta = get_viz_ds()
    _err  = get_viz_err()
    _theme = "dark" if map_theme.value else "light"
    ...
    if _err is not None:
        ...
        _controls = mo.vstack([
            mo.hstack([viz_field, viz_timestep, viz_depth, viz_colormap], justify="start"),
        ...
        try:
            ...
            _x0, _x1 = max(0, _x0), min(int(_nx), _x1)
            _y0, _y1 = max(0, _y0), min(int(_ny), _y1)
            ...
            # read() may return a generator; consume first result
            if not isinstance(_data, np.ndarray):
                _data = next(iter(_data))

            # Squeeze size-1 z dimension: (1,y,x) -> (y,x)
