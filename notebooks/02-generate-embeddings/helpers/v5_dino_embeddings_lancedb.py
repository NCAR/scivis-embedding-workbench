#!/usr/bin/env python3
# v5: rectangular image support
#
# Key differences from v4:
#   - Accepts --image_h and --image_w independently (both must be multiples of patch size).
#   - Preprocessing resizes directly to (image_h x image_w) with NO center-crop.
#     v4 used timm's create_transform which applies Resize + CenterCrop → square only.
#   - Model loaded with dynamic_img_size=True so positional embeddings adapt to
#     non-default patch grids without shape-mismatch errors.
#   - Token geometry uses (image_h // patch_size) * (image_w // patch_size).
#   - spatial_h and spatial_w are tracked separately in config and attention metadata.
#   - Mean / std / interpolation are resolved once in main and passed explicitly to
#     worker processes, avoiding a stub-model load per worker.
...
def _remove_stale_config_keys(db, config_table_name: str, keys: List[str]) -> None:
    """Delete specific keys from the config table (e.g. keys removed in a newer script version)."""
    try:
...
def build_rect_transform(mean, std, image_h: int, image_w: int, interpolation: str = "bicubic"):
    """
    Build a preprocessing transform that resizes to exactly (image_h, image_w).

    Unlike timm's create_transform (Resize → CenterCrop → Normalize), this pipeline
    is: Resize((H, W)) → ToTensor → Normalize. No cropping is performed, so
    rectangular inputs are preserved faithfully.
    """
    ...
    interp = interp_map.get(interpolation.lower(), InterpolationMode.BICUBIC)

    return transforms.Compose([
        transforms.Resize((image_h, image_w), interpolation=interp, antialias=True),
        transforms.ToTensor(),
        transforms.Normalize(mean=list(mean), std=list(std)),
    ])


def resolve_model_data_config(model_name: str) -> Dict[str, Any]:
    """Resolve timm data config (mean, std, interpolation) without loading pretrained weights."""
    import timm
    from timm.data import resolve_data_config

    stub = timm.create_model(model_name, pretrained=False, num_classes=0)
    cfg = dict(resolve_data_config({}, model=stub))
    del stub
    return cfg


def build_model(model_name: str) -> Any:
    """Load the pretrained model with dynamic_img_size=True for rectangular grid support."""
    import timm
    ...
    model = timm.create_model(
        ...
        num_classes=0,
        global_pool="avg",
        dynamic_img_size=True,  # REQUIRED: adapts positional embeddings to non-default patch grids
    )
    return model
...
def _worker_init(model_name: str, image_h: int, image_w: int, mean, std, interpolation: str) -> None:
    global _WORKER_PREPROCESS
    # mean/std/interpolation resolved once in main and passed here — no stub model load per worker
    _WORKER_PREPROCESS = build_rect_transform(mean, std, image_h, image_w, interpolation)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Compute DINOv2/v3 image + patch embeddings and write to LanceDB tables. "
            "v5: supports rectangular images — resizes to (image_h x image_w) without center-cropping. "
            "Both image_h and image_w must be multiples of the model's patch size."
        )
    )
    ...
    # Rectangular size args — must be provided together; both must be multiples of patch_size.
    # If neither is provided, the model's default square size is used.
    ap.add_argument("--image_h", type=int, default=None, help="Input height in pixels (must be a multiple of patch size)")
    ap.add_argument("--image_w", type=int, default=None, help="Input width in pixels (must be a multiple of patch size)")
    ...
    # Validate that image_h / image_w are either both set or both absent
    if (args.image_h is None) != (args.image_w is None):
        print("ERROR: --image_h and --image_w must be provided together.", file=sys.stderr)
        sys.exit(2)
    ...
    # Resolve data config (mean, std, interpolation) once — passed explicitly to workers
    data_cfg = resolve_model_data_config(args.model)
    mean = data_cfg["mean"]
    std = data_cfg["std"]
    ...
    if args.image_h is not None:
        ...
        image_mode = "rectangular" if image_h != image_w else "square"
    else:
        default_size = int(data_cfg["input_size"][1])
        image_h = default_size
        image_w = default_size
        image_mode = "square_default"

    # Load model with dynamic_img_size=True
    ...
    # Validate that both dimensions are patch-size multiples
    if image_h % patch_size_used != 0:
        print(
            f"ERROR: image_h={image_h} is not a multiple of patch_size={patch_size_used}.",
            file=sys.stderr,
        )
        sys.exit(2)
    if image_w % patch_size_used != 0:
        print(
            f"ERROR: image_w={image_w} is not a multiple of patch_size={patch_size_used}.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    # Determine token geometry with a rectangular dummy input
    model_dtype = next(model.parameters()).dtype
    with torch.no_grad():
        dummy = torch.zeros(1, 3, image_h, image_w, device=device, dtype=model_dtype)
    ...
    # Remove keys that existed in v4 but are no longer valid in v5,
    # in case this config table was previously written by v4.
    _remove_stale_config_keys(db_out, args.config_table, ["image_size_used"])
    ...
    kv.append(("image_h", str(image_h)))
    kv.append(("image_w", str(image_w)))
    kv.append(("image_mode", image_mode))
    ...
    kv.append(("interpolation", str(interpolation)))
    kv.append(("mean", str(mean)))
    kv.append(("std", str(std)))
    kv.append(("crop_pct", "none — v5 does not center-crop"))
    ...
    # Workers receive mean/std/interp directly — no stub model loaded per worker
    mp_ctx = mp.get_context("spawn")
    pool = mp_ctx.Pool(
        processes=args.workers,
        initializer=_worker_init,
        initargs=(args.model, image_h, image_w, mean, std, interpolation),
    )
    ...
    print("- spatial_grid: " + str(spatial_h) + "x" + str(spatial_w))
